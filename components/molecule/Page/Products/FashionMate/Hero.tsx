...
const Hero = () => {
  return (
    ...
    >
      <header className='w-full h-full flex flex-col justify-center items-start' >
        <h1 className="text-2xl md:text-4xl lg:text-5xl">Find Your Lifelong Fashion Mate</h1>
        <p className="text-gray-500 md:text-lg lg:text-xl">
          Unite with your perfect style companion for the ultimate Fashion exchange!
        </p>
      </header>
      <aside
      ...
      >
        ...
        <ul className="grid grid-cols-1 gap-1 md:gap-2">
          {fashionMateSteps.map((step, index) => (
            <li key={index} className="rounded-2xl p-3 bg-white shadow-md">
              <h4>{step.title}</h4>
